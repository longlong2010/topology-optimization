...
def quadElement(nodes, property):
    ...
    for p in points:
        ...
        for i in range(0, 4):
            ...
            B[2, i * 2] = Der[1, i];
            B[2, i * 2 + 1] = Der[0, i];
        Ke += B.T.dot(D).dot(B) * abs(numpy.linalg.det(J)) * w * t;
    return Ke;
...
if __name__ == '__main__':
    ...
    for s in range(0, 30):
        ...
        for jdx in range(0, N):
            ...
            j = jdx % (w - 1);  #列指标
            sum = 0.;
            for l in range(int(max(0, i - round(Rmin))), int(min(i + round(Rmin), h - 1))):
                for k in range(int(max(0, j - round(Rmin))), int(min(j + round(Rmin), w - 1))):
